...
| Item | Especificação |
|---|---|
| Processador | Intel Core i7-11800H @ 2,30 GHz (8 núcleos / 16 threads, 11ª geração) |
| Memória RAM | 16 GB |
| Sistema Operacional | Windows 11 Home (build 10.0.26200) |
| Compilador | gcc 15.2.0 (MSYS2) |
| Flags de compilação | `-O2 -Wl,--stack,67108864 -lm` |
...
- O Quicksort clássico é recursivo e, sobre vetor já ordenado, atinge
  profundidade O(n). Como o Windows não possui `ulimit`, a pilha foi ampliada
  para 64 MB via flag de linker (`-Wl,--stack,67108864`), equivalente ao
  `ulimit -s unlimited` recomendado no enunciado.
...
|---|---|---:|---:|---:|
| Bubblesort | Aleatório | 1,817428 | 449.885.077 | 225.521.470 |
| Bubblesort | Ordenado (melhor) | 0,000015 | 29.999 | 0 |
| Bubblesort | Inverso (pior) | 2,450625 | 449.985.000 | 449.985.000 |
| Insertionsort | Aleatório | 0,137092 | 225.551.463 | 225.521.470 |
| Insertionsort | Ordenado (melhor) | 0,000054 | 29.999 | 0 |
| Insertionsort | Inverso (pior) | 0,291188 | 449.985.000 | 449.985.000 |
| Selectionsort | Aleatório | 0,913152 | 449.985.000 | 29.982 |
| Selectionsort | Ordenado (melhor) | 0,932081 | 449.985.000 | 0 |
| Selectionsort | Inverso (pior) | 0,988081 | 449.985.000 | 15.000 |
...
O Quicksort foi, em média, **~61× mais lento** que o Heapsort neste cenário, e
...
|---|---:|---:|---:|
| Insertionsort | 0,000077 | 50.247 | 248 |
| Shellsort | 0,000790 | 700.254 | 248 |

_(dados brutos em `data/problema3.csv`)_

O Insertionsort foi **~10× mais rápido** e fez **~14× menos comparações** que o
Shellsort, embora ambos tenham realizado praticamente o mesmo número de trocas
(248).
...
**0,137 s contra 0,913 s** do Selectionsort — cerca de **6,7× mais veloz**. A
...
- **Insertionsort é adaptativo.** O laço interno (`while`) tem um `break`: assim
  ...
  encontrar seu lugar. O resultado empírico confirma isso: **225.551.463**
  comparações — praticamente metade (`0,501×`) das 449.985.000 do Selectionsort.
...
trocas** (225.521.470) que o Selectionsort (29.982). Ainda assim foi mais
...
**~61× menor**. O *heap* não tem um "caso degenerado" análogo ao do pivô fixo do
...
(0,077 ms contra 0,790 ms — ~10× mais rápido), e fez **50.247 comparações**
contra **700.254** do Shellsort.
...
- O **Insertionsort é adaptativo**: seu custo é O(n + d), onde `d` é o número de
  ...
  ordem e para (`break`). Por isso as comparações (50.247) ficam logo acima de
  `n = 50.000`: comportamento praticamente **linear**. As 248 trocas
...
O detalhe revelador é que **ambos fizeram 248 trocas**: o vetor exigia pouquíssima
